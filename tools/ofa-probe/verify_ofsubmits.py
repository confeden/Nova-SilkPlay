"""Does the ~17-execute temporal-hint device-lost limit reset across submits?
Records ONE command buffer with N executes and submits it S times, waiting idle
between submits. Total chained executes = N*S.
Usage: python verify_ofsubmits.py <perf> <grid> <N per cb> <S submits> <execFlags>
"""
import sys
from ctypes import *
vk = CDLL(r"C:\Windows\System32\vulkan-1.dll")
PERF=int(sys.argv[1]); GRID=int(sys.argv[2]); N=int(sys.argv[3]); S=int(sys.argv[4]); EF=int(sys.argv[5])
W,H=2560,1440
...
def mkimg(fmt,w,h,u):
    ...
    im=c_uint64(); vk.vkCreateImage(dev,byref(ic),None,byref(im))
...
sess=c_uint64(); r=mkS(dev,byref(sci),None,byref(sess)); assert r==0,r
for bp,v in [(1,v1),(2,v2),(4,v3)]: bnd(dev,sess,bp,v,1)
...
print("N=%d per cb, S=%d submits, execFlags=%d -> total chained executes %d"%(N,S,EF,N*S))
for s in range(S):
    rs=vk.vkQueueSubmit(qq,1,byref(si),0); rw=vk.vkQueueWaitIdle(qq)
    if rs!=0 or rw!=0:
        print("  DEVICE LOST at submit #%d (cumulative executes=%d) submit=%d waitIdle=%d"%(s+1,(s+1)*N,rs,rw)); sys.exit(1)
print("  ALL %d submits OK, %d cumulative executes, no device loss"%(S,N*S))
