...
SF=int(sys.argv[8]) if len(sys.argv)>8 else 0
...
sci=OFSCI(s=1000464004,p=None,w=W,h=H,imf=FMT_BGRA,fvf=FMT_S105,cf=(13 if SF&2 else 0),og=GRID,hg=HG,perf=PERF,flags=SF)
sess=c_uint64(); r=mkS(dev,byref(sci),None,byref(sess))
print("createSession(perf=%d grid=%d hintGrid=%d) -> %d"%(PERF,GRID,HG,r))
if r!=0: sys.exit(1)
binds=[(1,v1),(2,v2),(4,v3)]
extra=[]
if SF&16:
    ib,vb=mkimg(FMT_S105,fw,fh,2); extra.append(ib); binds.append((5,vb))       # BACKWARD_FLOW_VECTOR
if SF&2:
    ic1,vc1=mkimg(13,fw,fh,8); extra.append(ic1); binds.append((6,vc1))          # COST (R8_UINT)
    if SF&16:
        ic2,vc2=mkimg(13,fw,fh,8); extra.append(ic2); binds.append((7,vc2))      # BACKWARD_COST
for bp,v in binds:
    rb=bnd(dev,sess,bp,v,1)
    if rb!=0: print("  bind bp=%d -> %d"%(bp,rb))
...
bars=(IMB*(3+len(extra)))(*[IMB(s=45,p=None,sa=0,da=0,ol=0,nl=1,sq=0xffffffff,dq=0xffffffff,img=i,sr=SRR(1,0,1,0,1)) for i in [i1,i2,i3]+extra])
vk.vkCmdPipelineBarrier(cb,0x10000,0x10000,0,0,None,0,None,3+len(extra),bars)
...
if rw==0:
    ...
    print("  sessFlags=%d queryResults=%d  %dx%d grid%d -> flow %dx%d  median=%.3f ms  min=%.3f  p95=%.3f"%(
        SF,rq,W,H,GRID,fw,fh,ts[ITER//2],ts[0],ts[int(ITER*0.95)]))
